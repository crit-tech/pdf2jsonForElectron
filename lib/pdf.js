    ...
    parsePage(callback, errorCallBack) {
        ...
        function pageViewDrawCallback(error) {
            ...
            if (error) {
                ...
                console.info(`Success: Page ${this.id + 1}`);
                callback();
            }
        }
...
export default class PDFJSClass extends EventEmitter {
    ...
    tryLoadFieldInfoXML(pdfFilePath) {
        ...
        console.info("About to load fieldInfo XML : " + fieldInfoXMLPath);
        ...
        this.ptiParser.parseXml(fieldInfoXMLPath, err => {
            if (err) {
                ...
                console.info("fieldInfo XML loaded.");
            }
        });
    }
	...
	loadPages() {
        ...
        console.info("PDF loaded. pagesCount = " + pagesCount);
    ...
    parsePage(promisedPages, id, scale) {
        console.info("start to parse page:" + (id+1));
        ...
        function continueOnNextPage() {
            console.info("complete parsing page:" + (id+1));
	            ...
	            if (this.needRawText) {
	                pdfPage.getTextContent().then(
			            textContent => {
                            ...
                            console.info("complete parsing raw text content:" + (id+1));
	...
	getMergedTextBlocksIfNeeded() {
		for (let p = 0; p < this.pages.length; p++) {
			...
			page.Texts = page.Texts.filter( (t, j) => {
				...
				if (isDup) {
                    console.info("skipped: dup text block: " + decodeURIComponent(t.R[0].T));
				}
				return !isDup;
			});
			...
			for (let i = 0; i < page.Texts.length; i++) {
				...
				if (prevText) {
					if (PDFFont.areAdjacentBlocks(prevText, text) && PDFFont.haveSameStyle(prevText, text)) {
                        ...
                        console.info(`merged text block: ${preT} + ${curT} => ${mergedText}`);
