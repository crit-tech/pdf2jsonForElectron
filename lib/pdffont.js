    ...
    #initSymbol() {
        ...
        if (this.fontObj.isSymbolicFont) {
            ...
            if (mFonts.length > 0) {
                ...
                console.info("Reset: isSymbolicFont (false) for " + this.fontObj.name);
            }
        }
        else {
            if (isSymbol) {
                this.fontObj.isSymbolicFont = true; //text pdf: va_ind_760c
                console.info("Reset: isSymbolicFont (true) for " + this.fontObj.name);
            }
        }
        return isSymbol;
    }
...
//        nodeUtil.p2jinfo"typeName = " + typeName + " => faceIdx = " + this.faceIdx);
    ...
    #processSymbolicFont(str) {
        ...
        switch(str.charCodeAt(0)) {
            ...
            default:
                console.info(this.fontObj.type + " - SymbolicFont - (" + this.fontObj.name + ") : " +
    ...
    processText(p, str, maxWidth, color, fontSize, targetData, matrix2D) {
        ...
        if (rAngle != 0) {
            console.info(str + ": rotated " + rAngle + " degree.");
